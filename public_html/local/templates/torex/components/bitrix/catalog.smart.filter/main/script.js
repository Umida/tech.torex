	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			if (result.INSTANT_RELOAD && result.COMPONENT_CONTAINER_ID)
			{
				...
				url = BX.util.htmlspecialcharsback(result.FILTER_AJAX_URL);
				ajax_url = BX.util.remove_url_param(url,["bxajaxid"]);
				history.replaceState(null, "", ajax_url)
				BX.ajax.insertToNode(url, result.COMPONENT_CONTAINER_ID);
			}
			else
			{
			    
			    inputtop = $(this.curFilterinput).parent().position().top - 10;
			    console.log(inputtop);
			    
			    $('.smart-filter-container-modef').css({ top: inputtop+'px' });
...
$(document).ready(function(){
    if ($('.smart-filter').attr("data-count") > 0)
        $('.filter_count').html('<span>'+$('.smart-filter').attr("data-count")+'</span>');
	$(document).on('click','.sort__option',function(e){
		if ($('.dropdown__options').hasClass('active')){
			$('.dropdown__options').removeClass('active')
		}
		else{
			$('.dropdown__options').addClass('active')
		}
