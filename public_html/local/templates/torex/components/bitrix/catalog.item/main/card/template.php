  ...
  <div class="prods-line_item-image">
       ...
       <div class="prods-line_item-desc" id="<?=$itemIds['PROP_DIV']?>">
                ...
                <div class="item-desc_sku">
					...
					<button class="catalog-section-item-fav-btn<?=$wishclass?>" onclick="add2wish('<?=$wishaction?>','<?=($wishclass==''?$item['ID']:array_search($item['ID'], $delaydBasketItems))?>', '<?=$price["PRICE_TYPE_ID"]?>', '<?=$price["PRINT_RATIO_PRICE"]?>', '<?=$item["NAME"]?>', '<?=$item['DETAIL_PAGE_URL']?>', this)" data-productid="<?=$item['ID']?>"><i data-svg="fav"></i>
					</button>
